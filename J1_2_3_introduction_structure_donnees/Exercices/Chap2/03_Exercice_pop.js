...
const isPresent = letter => ["a", "l"].includes( letter.toLowerCase()) ? 1 : 0

const letterCount = (person) => {
     const {name} = person;
     const n = person.name;
     const letters = [...name];
     const count = letters.reduce((acc, letter) => acc + isPresent(letter) , 0);
     person.count = count;

     return person;
 }

 const electedPersons = populations
                .map(letterCount)
